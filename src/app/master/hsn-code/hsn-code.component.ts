...
export class HsnCodeComponent implements OnInit {
  ...
  constructor(private _formBuilder : FormBuilder, private _masterService : MasterService,
  ...
  ngOnInit(): void {
    ...
    this.hsnForm = this._formBuilder.group({
      ...
      CREATED_BY:[''],
      CREATED_DATE:[''],
      CODE_LIST: new FormArray([]),
    });



    const add = this.hsnForm.get('CODE_LIST') as FormArray;
    add.push(
  ...
  Search(){
    var FROM_DATE =  this.hsnForm1.value.FROM_DATE == null   ? ''  : this.hsnForm1.value.FROM_DATE;
    var TO_DATE =  this.hsnForm1.value.TO_DATE == null  ? ''  : this.hsnForm1.value.TO_DATE;
 ...
 this.isLoading = true;
  ...
  SubmitHSN(){
    ...
    if (this.hsnForm.invalid) {
      ...
      this.hsnForm.get("CREATED_BY").setValue(this._commonService.getUserName());
      this.hsnForm.get("CREATED_DATE").setValue(this._commonService.getcurrentDate(new Date()))
      this._masterService.InsertHsnCode(JSON.stringify(this.hsnForm.value)).subscribe((res:any)=>{
  ...
  ClearForm(){
    ...
    this.hsnForm.get('HSN_CODE')?.setValue('');
    this.hsnForm.get('HSN_DESC')?.setValue('');
    this.hsnForm.get('CREATED_BY')?.setValue('');
    this.hsnForm.get('CREATED_DATE')?.setValue('');
  }
